...
const useIntersectionContext = () => {
  ...
  if (context === null)
    throw new Error("useIntersectionContext used outside IntersectionContext");
  return context;
};
...
const useIntersectionObserver = <T extends Element>({
  ...
  useEffect(() => {
    ...
    if (elementRef.current)
      addIntersctionItem(elementRef.current, {
        handleIntersect,
        threshold,
        unobserveOn,
      });
