import { SliderProvider, Slider, SliderPrev, SliderNext, SliderPagination } from '@/components/Slider/Slider'
...
export default function ReviewSlider({ className = "", reviews }: Props) {
    const items = reviews.map(({ author, authorSrc, bg, company, reviewText }, idx) =>
        <div key={idx} className="relative text-center text-xl lg:text-11xl h-full w-full">
            <Image width={30} height={30} className="absolute top-0 w-full h-full -z-[1] object-cover" alt="" src={bg} />
            <div className="flex flex-col items-center justify-center h-full">
                <article className="p-8 md:p-20 flex flex-col-reverse gap-5 lg:gap-10 items-center justify-center h-full" aria-label={`Review by ${author} from ${company}`}>
                    <div className="flex flex-row items-center justify-start gap-[20px] text-md lg:text-5xl font-raleway">
                        <Image width={20} height={20} className="w-16 h-16 bg-black rounded-[100px] object-cover" alt="" src={authorSrc} />
    ...
    return (
        <div className={className} onMouseEnter={() => setMouseIn(true)} onMouseLeave={() => setMouseIn(false)}>
            <SliderProvider autoplay={!mouseIn}>
                ...
                {/* <SliderNavigation /> */}
                <div className={(mouseIn?"opacity-100":"opacity-0") + " transition-opacity"}>
                    <SliderPrev />
                    <SliderNext />
                </div>
