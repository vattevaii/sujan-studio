...
function Service({ service }: { service: ServiceItem }) {
    ...
    return <div ref={itemRef} className="w-full relative text-[30px] md:text-21xl xl:text-41xl">
        <div className='absolute z-[1] group py-[1em] pointer-coarse:px-5 flex flex-col pointer-coarse:flex-row 
        ...
        <Image width={500} height={300} sizes="(max-width:560px)100vw,50vw" className={(visibleImg ? "" : "hidden") + " absolute z-0 top-0 left-0 w-full h-auto object-cover"} alt="" src={service.src} />
    </div>
}
